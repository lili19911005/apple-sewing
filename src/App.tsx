...
function ProjectionFeature({ pages, files, direction, perLine, horizontalGap, verticalGap, onHorizontalGap, onVerticalGap }: { pages: EditablePdfPage[]; files: File[]; direction: 'horizontal' | 'vertical'; perLine: number; horizontalGap: number; verticalGap: number; onHorizontalGap: (gap: number) => void; onVerticalGap: (gap: number) => void }) {
  ...
  const projectionLineRef = useRef<SVGSVGElement>(null)
  ...
  function pointInProjection(event: React.PointerEvent<HTMLDivElement>) {
    const lineTransform = projectionLineRef.current?.getScreenCTM()
    if (lineTransform) {
      const point = new DOMPoint(event.clientX, event.clientY).matrixTransform(lineTransform.inverse())
  ...
  function movePatternAlongSelectedLine() {
    ...
    if (!length) return
    const moveX = dx / length * length
    const moveY = dy / length * length
    setPatternOffset((current) => ({ x: current.x + moveX, y: current.y + moveY }))
    setLines((old) => old.map((current) => ({ start: { x: current.start.x + moveX, y: current.start.y + moveY }, end: { x: current.end.x + moveX, y: current.end.y + moveY } })))
  }
  ...
  function selectAdjacentLine(directionStep: -1 | 1) {
    ...
    const nextIndex = (selectedLineIndex + directionStep + lines.length) % lines.length
    setSelectedLineIndex(nextIndex)
    focusLineOnScreen(nextIndex)
  }

  function focusLineOnScreen(index: number) {
    const line = lines[index]
    const matrix = projectionLineRef.current?.getScreenCTM()
    const displayRect = projectionDisplayRef.current?.getBoundingClientRect()
    if (!line || !matrix || !displayRect) return
    const center = new DOMPoint((line.start.x + line.end.x) / 2, (line.start.y + line.end.y) / 2).matrixTransform(matrix)
    const target = { x: displayRect.left + displayRect.width / 2, y: displayRect.top + displayRect.height / 2 }
    setPan((current) => ({ x: current.x + target.x - center.x, y: current.y + target.y - center.y }))
  }
